...
function EducationCard({ item, index }) {
  ...
  return (
    <motion.div
      ...
      `}
      style={{
        background:  item.highlight ? 'var(--color-surface)' : 'var(--color-bg-card)',
        borderColor: item.highlight ? 'var(--color-accent-dim)' : 'var(--color-border)',
      }}
    >
      <div
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          <div className="text-xs font-semibold text-theme">{FOOTBALL.passion}</div>
          <div className="text-[10px] font-mono text-theme-faint">{FOOTBALL.positions}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        {Object.entries(FOOTBALL.stats).map(([k, v]) => (
          <div key={k} className="text-center">
            ...
            <div className="text-[9px] font-mono text-theme-faint mt-0.5 leading-tight">{k}</div>
          </div>
        ))}
      </div>

      <p className="text-[11px] font-mono text-theme-muted italic leading-relaxed border-l-2 pl-3"
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div>
            ...
            >
              {PERSONAL.bio.map((para, i) => (
                <p key={i} className="text-sm text-theme-muted leading-relaxed mb-4">
                  {para}
                </p>
              ))}
            </motion.div>
            ...
            >
              {[
                ...
                { label: 'GitHub',   value: 'ARPANkundu2404',  href: PERSONAL.github  },
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  {item.href ? (
                    ...
                    <span className="text-xs font-mono text-theme-muted">{item.value}</span>
                  )}
                </div>
              ))}
            </motion.div>
            ...
            {/* Football easter egg */}
            <div className="mt-8">
              <div className="section-label text-[9px] mb-3">OUTSIDE THE IDE</div>
              <FootballSection />
            </div>
          </div>
          ...
          <div>
            ...
            >
              <div className="section-label text-[9px] mb-3">RELEVANT COURSEWORK</div>
              <div className="flex flex-wrap gap-2">
                {['Digital Electronics', 'Microprocessors', 'Network Theory',
                  'Signals & Systems', 'Internet of Things', 'VLSI Design'].map(c => (
